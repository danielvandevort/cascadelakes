...
lakes = ee.FeatureCollection("path/to/lake/data")

# Define the time period
startDate = '2014-01-01'
endDate = '2024-06-30'
num_lakes = 2897 # change as needed
...
def calc_data(image, currentLake):
    ...
    values = combined.reduceRegion(  # this returns an ee.Dictionary
        ...
        scale=30  # HLS2 imagery is 30m resolution
    )
    # each is an ee.List object
    ndsi_values = values.get('NDSI')  # assumed that all pixels are in the same order...

    def create_feature(i):
        return ee.Feature(None, {
            'date': image.date().format('YYYY-MM-dd'),
            'lake_id': currentLake.get("ObjectID_1"),
            'NDSI': ee.List(values.get("NDSI")).get(i),
            'NIR_S': ee.List(values.get("NIR_S")).get(i), # Sentinel-2 NIR band
            'NIR_L': ee.List(values.get("NIR_L")).get(i)  # Landsat NIR band
        })
...
for i in range(0, num_lakes, batch_size):
    ...
    export_task = ee.batch.Export.table.toDrive( # goes to a Google Drive folder
        collection=results,
        description=f'HLS_NDSI_NIR_batch_{i}',
        folder='HLS_NDSI_NIR',
        fileFormat='CSV',
...
# test = lakes.map(process_lakes).flatten()
# export_test_combined = ee.batch.Export.table.toDrive(
#     collection=test,
#     description=f'NDSI_NIR_test',
#     folder='NDSI_NIR_test',
#     fileFormat='CSV',
#     selectors=['date', 'lake_id', "NDSI", "NIR_L", 'NIR_S']
# )
# export_test_combined.start()
